...
- `POST /api/claim/verify` 是公开领取页唯一需要调用的公开 API。
- 口令错误、口令未配置、订阅未生成、订阅已过期或公开订阅域名未配置时，不返回 `copyableSubscriptionUrl`。
- 口令正确且订阅可领取时，返回 `copyableSubscriptionUrl`。
- `copyableSubscriptionUrl` 是完整公开订阅地址，由 `publicSubscriptionBaseUrl + safeSubscriptionUrl` 拼接。
- `/claim` 页面收到 `copyableSubscriptionUrl` 后只用于复制到剪贴板，不渲染为文本。
- `/claim` 页面不再调用 `/api/subscription/status`。
- 后续公开域名只需要放行 `/claim`、`/assets/*`、`/api/claim/verify` 和 `/sub/*`。
- 继续禁止 `/api/subscription/status`、`/api/status`、其他 `/api/*`、后台页面和根路径策略外的入口。

## v0.8.2 领取防刷和二维码隐藏规则

- `POST /api/claim/verify` 使用内存 Map 按客户端 IP 记录口令失败次数。
- IP 识别优先级为 `x-forwarded-for` 第一个 IP、`x-real-ip`、请求 IP 或 socket IP，无法获取时使用 `unknown`。
- 默认在 15 分钟窗口内允许 5 次失败，达到阈值后锁定 15 分钟。
- 冷却期内返回 HTTP 429 和 `CLAIM_TOO_MANY_ATTEMPTS`，不判断口令是否正确，也不返回订阅链接。
- 口令正确后清除该 IP 的失败计数和锁定状态。
- `copyableSubscriptionUrl` 仍只在口令正确且订阅可领取时返回。
- 后端不记录用户输入的完整口令，不返回 `CLAIM_ACCESS_CODE`。
- 订阅管理页不直接展示二维码图片，只显示二维码状态和“下载二维码”按钮。
- 点击下载二维码时使用公开订阅地址临时生成 PNG，文件名为 `huage-secure-subscription-qr.png`。
- 页面不显示完整订阅链接、`/sub/{token}`、token、raw 节点或 raw/base64 内容。

## v0.8.3 发布前检查规则

- 发布前检查接口为 `GET /api/publish-check/status`。
- 检查订阅是否生成、订阅是否过期、剩余天数、订阅节点数量、公开订阅域名、复制链接准备状态和领取口令配置。
- 检查公开领取页 `/claim` 是否可访问，检查 `/api/claim/verify` 是否可用。
- 检查公开域名下 `/api/status`、`/api/subscription/status`、`/api/node-pool/status`、`/api/detection/xray/status` 必须返回 404。
- 如果存在失败项，`canPublish=false`；只有警告项时 `canPublish=true` 且 `level=warning`；全部通过时 `level=pass`。
- 接口只返回安全摘要，不返回 token、完整订阅链接、真实口令、raw 节点或 raw/base64 内容。
# v0.9.0 发布候选版订阅规则

- 正式发布前必须确认 `CLAIM_ACCESS_CODE` 是本期视频口令。
- 正式发布前建议执行发布前准备，重置安全订阅 token 并续期。
- 重置 token 后必须重新下载二维码。
- 公开订阅域名只能开放 `/claim`、`/assets/*`、`/api/claim/verify` 和 `/sub/*`。
- 公开域名不能开放全部 `/api/*`，后台管理地址和 3000 端口不能发给粉丝。
- 文档、脚本、页面和接口都不能输出 token 全文、完整订阅链接、真实口令、raw 节点或 base64 订阅内容。
- 最终验收脚本只做只读检查，不重置 token、不续期、不修改 Caddy、不修改 `data/`。

# v1.0.5 手动验证可用节点规则

- 订阅候选节点包括自动检测 `status=available` 的节点。
- 订阅候选节点也包括 `manualOverride=true` 且 `manualStatus=available` 的节点。
- `POST /api/node-pool/mark-manual-available/:nodeId` 用于管理员在 VPS 手动验证节点可用后，将节点写入可订阅状态。
- 手动验证可用节点写入 `detectionCore=manual`，页面必须清楚显示不是自动检测可用。
- 不可用、暂不支持、错误、待检测以及 `manualStatus=unavailable` 的节点不能进入订阅。
- 订阅接口和页面仍不能显示 raw 节点、token 或完整订阅链接。

# v1.0.6 发布管理规则

- 后台“发布管理 / 本期领取设置”可以集中显示本期领取口令、公开领取页、订阅状态、可用节点数、有效期、最后生成时间和操作历史。
- 管理后台允许显示当前领取口令，但必须标注“仅后台可见，不要公开后台地址”。
- 口令可手动设置或随机生成，写入运行状态文件后立即生效，服务重启后仍保留。
- 发布管理页不显示完整订阅 token，不裸露完整订阅链接；如需展示订阅链接，只显示 `https://get.huage.us/sub/***masked***`。
- 发布管理页可以提供复制公开领取页、复制安全订阅链接、刷新订阅和重置 token 操作，但页面不能显示 raw 节点或 raw/base64 链接。
