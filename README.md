...
当前版本为 `v1.1.0` 自动化运行总控版，已经完成项目骨架、GitHub 线索搜索、节点解析、本地 JSON 节点池基础、Xray-core 检测、节点状态手动校正、安全订阅链接、订阅缓存自动刷新、订阅页隐藏 token 展示、公开订阅域名配置、安全订阅 token 重置、订阅有效期、订阅二维码下载、公开领取页和口令验证、领取验证成功后返回可复制订阅链接、领取口令防刷、隐藏二维码预览、发布前检查页、发布前检查结果优化、发布前一键准备操作区、Xray-core 内核状态识别热修复、Xray 检测流程与手动成功流程对齐、单节点 Xray 检测结果写回修复、`test-node/:nodeId` 写回后重读验证、手动验证可用节点加入订阅池机制、后台发布管理 / 本期领取设置页面，以及自动化运行总控。
...
`v1.1.0` 新增“自动化运行”后台页面和 `/api/automation/*` 接口。管理员开启总开关后，服务会按配置间隔自动编排现有能力：GitHub 搜索线索、解析入池、低并发 Xray 检测未测试节点，并在需要时刷新订阅。

自动化默认关闭，运行间隔默认 30 分钟。每轮最多搜索 3 个关键词、解析入池最多 20 条节点、检测最多 5 条未测试节点，Xray 检测并发保持 1。运行状态和最近日志写入 `data/automation_state.json`，属于运行数据，不提交 Git。自动化日志只保存脱敏摘要，不保存 raw 节点、订阅 token、完整订阅链接或 GitHub Token。
